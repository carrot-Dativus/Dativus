...
직접 설치하는 과정을 촬영한 영상입니다. 아래 링크를 참고하시면 더 쉽게 따라하실 수 있습니다.

| 구분 | 링크 |
|---|---|
| 설치 가이드 영상 | [▶ YouTube 링크](https://youtube.com/링크를_여기에_입력) |
...
4. [로컬 LLM 설치 (Ollama + qwen2.5:14b)](#4-로컬-llm-설치-ollama--qwen2514b)
...
[브라우저]
    ...
    └─── SSE 스트리밍 ────────► [FastAPI AI 서버 :8000]
                                    ...
                                    │
                                    ├─ Ollama qwen2.5:14b (로컬 LLM · Groq 한도 초과 시 자동 폴백)
                                    │
...
- 다운로드: https://neo4j.com/deployment-center/ → `Graph Database Self-Managed` → `Community` 탭 → Windows 설치 파일
- 설치 후 http://localhost:7474 접속 → 초기 로그인: ID `neo4j` / 비밀번호 `neo4j`
- 첫 로그인 시 비밀번호 변경 요구 → **`1234`** 로 설정 (FastAPI .env와 일치해야 합니다)
- 설치 확인: http://localhost:7474 에서 Neo4j Browser가 열리면 정상
...
## 4. 로컬 LLM 설치 (Ollama + qwen2.5:14b)

Ollama는 역질문 생성(Clarification) 및 Groq API 한도 초과 시 자동 폴백 역할을 합니다. **역질문 기능이 항상 Ollama를 사용하므로 반드시 설치해야 합니다.**
...
**② qwen2.5:14b 모델 다운로드** (약 9GB, 최초 1회)
```
...
> **⚠️ 최초 실행 시 — BAAI/bge-m3 임베딩 모델 자동 다운로드**  
> FastAPI 첫 실행 시 BAAI/bge-m3 모델(약 1.5GB)이 자동으로 다운로드됩니다.  
> 인터넷 연결이 필요하며 다운로드 완료까지 수 분이 소요됩니다. 이후 실행부터는 캐시에서 빠르게 로드됩니다.
...
- **해결**: 자동으로 로컬 Ollama로 전환됩니다. Ollama와 qwen2.5:14b가 설치되어 있어야 합니다 (4번 항목 참조)
...
| `Dativus_Ai/.env` | `OLLAMA_BASE_URL` | `http://localhost:11434` | Ollama 서버 주소 |
| `Dativus_Ai/.env` | `NEO4J_URI` | `bolt://localhost:7687` | Neo4j Bolt 접속 주소 |
| `Dativus_Ai/.env` | `NEO4J_USER` | `neo4j` | Neo4j 계정 |
| `Dativus_Ai/.env` | `NEO4J_PASSWORD` | `1234` | Neo4j 비밀번호 |
